// Sidebar - Demo Mode and quick scenario controls (FR-O-004)
const Sidebar = () => {
    ...
    <div className="card" style={{ padding: 22, height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: "-0.02em", marginTop: 8 }}>10분 자동 시나리오</div>
          <div style={{ fontSize: 11.5, color: "#6B7280", marginTop: 2 }}>FR-O-004 · M+3 LH 본사 시연용</div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ height: 4, background: "#F4F5F7", borderRadius: 999, overflow: "hidden" }}>
          <div style={{ height: "100%", width: "43%", background: "linear-gradient(90deg, #34D399, #10B981)" }}/>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "grid", gap: 4, overflow: "auto" }}>
        {steps.map((s, i) => (
          <div key={i} style={{
            display: "grid", gridTemplateColumns: "16px auto 1fr",
            gap: 10, alignItems: "center", padding: "8px 4px",
            borderRadius: 8,
            background: s.active ? "#F0FDF4" : "transparent",
          }}>
            <span style={{
              width: 14, height: 14, borderRadius: 999,
              background: s.done ? "#10B981" : s.active ? "#fff" : "#fff",
              border: s.active ? "2px solid #10B981" : `1.5px solid ${s.done ? "#10B981" : "#E2E5EA"}`,
              display: "grid", placeItems: "center",
              color: "#fff", flexShrink: 0,
            }} className={s.active ? "pulse-dot" : ""}>
              {s.done && <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"><path d="m5 12 5 5L20 7"/></svg>}
            </span>
      ...
      <div style={{ marginTop: 14, display: "flex", gap: 8 }}>
        <Btn variant="primary" size="md" style={{ flex: 1, justifyContent: "center" }} onClick={() => setRunning(!running)}>
          {running ? "⏸ 일시정지" : "▶ 시연 재생"}
        </Btn>
